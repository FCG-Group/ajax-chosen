...
return _this.chosenXhr=$.ajax(options);},options.afterTypeDelay);};ajaxChosen.prototype.show_results=function(data){var _this,items,nbItems,selected_values,val_before_trigger;if(data==null){return;}
...
if($.inArray(value+"-"+text,selected_values)===-1){return $("<option />").attr('value',value).html(text).appendTo(_this.element);}}});if(nbItems){val_before_trigger=this.search_field.val();this.element.trigger("chosen:updated");this.search_field.val(val_before_trigger);}else{this.element.data().chosen.no_results_clear();this.element.data().chosen.no_results(this.search_field.val());}
